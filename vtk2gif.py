...
main(vtk_file_path, [save_path=vtk_path, datatitle='vdata',
    size=(1000,1000), showaxes=True, colorBounds=(), time_path='',
...
time_path: A string containing the file path and name of the '.txt'
...
def main(*arg, **kwargs):
    ...
    if len(arg) == 0:
        ...
        vtk_file_path = arg[0]
        vtk_path, vtk_name = os.path.split(vtk_file_path)
    ...
    # can be a list of integers, a list of sublists of integers, or a string of a file path
    ...
    if len(selectframes)>0:
        ...
        if isinstance(selectframes, basestring):
    ...
    if len(colorBounds) > 0:
        ...
        for vtknum in tqdm(range(numvtks), desc='Finding data minimum and maximum'):
            # Set vtk name in case of many vtks
            if numvtks == 1:
                temp_vtk_name = vtk_name
                temp_vtk_path = vtk_file_path
            else:
                temp_vtk_name = vtk_name[:-4]+'_'+str(vtknum)+vtk_name[-4:]
                temp_vtk_path = os.path.join(vtk_path, temp_vtk_name)
            
            # Load vtk
            datalist = []
            src = _load_vtk(temp_vtk_path, engine, useCBar=False)
            module_manager = src.children[0]
            
            # Get global min/max values of this vtk
            # Loop through all frames
            for vdataID in tqdm(datalist, desc='Finding local min/max of '+temp_vtk_name):
    ...
    if len(select_frame_list) == 0:
        for vtknum in tqdm(range(numvtks), desc='Generating GIF(s)'):
            ...
            if numvtks == 1:
                ...
                temp_vtk_name = vtk_name[:-4]+'_'+str(vtknum)+vtk_name[-4:]
                temp_vtk_path = os.path.join(vtk_path, temp_vtk_name)
            gif_name = temp_vtk_name[:-3]+'gif'
            gif_path = os.path.join(save_path, gif_name)
            
            # Load vtk
            datalist = []
            src = _load_vtk(temp_vtk_path, engine, useCBar=True)
            module_manager = src.children[0]
            ...
            with imageio.get_writer(gif_path, mode='I', fps=fps) as writer:
                for frameIDX in tqdm(datalist, desc='Generating GIF: '+gif_name):
        ...
        if numvtks == 1:
            ...
            vtknum = select_frame_list[0][0]/maxframes
            temp_vtk_path = vtk_file_path[:-4]+'_'+str(vtknum)+vtk_file_path[-4:]
        ...
        for gif_list in tqdm(select_frame_list, desc='Generating GIF(s)'):
            ...
            gif_name = vtk_name[:-4]+' - frames '+str(gif_list[0])+'-'+str(gif_list[-1])+'.gif'
            gif_path = os.path.join(save_path, gif_name)
            with imageio.get_writer(gif_path, mode='I', fps=fps) as writer:
                
                # Loop through each frame in sublist
                #for vdataID in tqdm(datalist[skipframes:], desc='Converting '+vtk_name+' to GIF'):
                for frameIDX in tqdm(gif_list, desc='Generating GIF: '+gif_name):
                    ...
                    if numvtks == 1:
                        ...
                        temp_vtknum = (frameIDX-1)/maxframes
...
if __name__ == '__main__':
    ...
    vtk_file_path = tkFileDialog.askopenfilename()
